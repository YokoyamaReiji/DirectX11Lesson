...
void EditorCamera::Update()
{
	...
	{
		//マウスの前回からの移動量
		float deltaX = (float)(nowPos.x - m_prevMousePos.x) * 0.6f * ratio;
		float deltaY = (float)(nowPos.y - m_prevMousePos.y) * 0.6f * ratio;
	...
	if (APP.m_window.GetMouseWheelVal())
	{
		...
		mDistance = DirectX::XMMatrixTranslation(0.f, 0.f, (float)APP.m_window.GetMouseWheelVal() * 0.01);
